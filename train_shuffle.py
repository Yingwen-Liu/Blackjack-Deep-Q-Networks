...
class DQNAgent:
    ...
    def save_model(self, path="blackjack_shuffle_dqn.pth"):
        torch.save({
    ...
    def load_model(self, path="blackjack_shuffle_dqn.pth"):
        import os
...
def encode_state(player_hand, dealer_revealed_card):
    """
    ...
    state = [0]*(2-len(player_points)) + player_points + [0]*(2-len(dealer_points)) + dealer_points     # 2 + 2
    
    return state


def train(agent, episodes=6000):
    deck = Deck(amount=1, shuffle=True)
    score = Score()
    ...
    with tqdm(total=episodes, desc="Training") as pbar:
        for _ in range(episodes):
            ...
            state = encode_state(player_hand, dealer_revealed_card)
            ...
            while not done:
                ...
                next_state = encode_state(player_hand, dealer_revealed_card)
...
def test(agent, episodes=50000):
    deck = Deck(amount=2, shuffle=True)
    score = Score()
